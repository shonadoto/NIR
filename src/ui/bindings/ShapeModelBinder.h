...
/**
 * @brief Binds ShapeModel instances to existing scene objects (ISceneObject/QGraphicsItem)
 *        and keeps properties (name, color/material) in sync.
 */
...
public:
    ...
    std::shared_ptr<ShapeModel> attach_shape(ISceneObject *item, const std::shared_ptr<ShapeModel> &model);
...
private:
    ...
    void update_model_geometry(ISceneObject *item, const std::shared_ptr<ShapeModel> &model);
    void apply_geometry(ISceneObject *item, const std::shared_ptr<ShapeModel> &model);
