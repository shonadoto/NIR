...
class MaterialModel : public ModelObject {
 public:
  ...
  double grid_frequency_x() const {
    return grid_frequency_x_;
  }
  void set_grid_frequency_x(double frequency);
  double grid_frequency_y() const {
    return grid_frequency_y_;
  }
  void set_grid_frequency_y(double frequency);

  // Legacy method for backward compatibility
  double grid_frequency() const {
    return grid_frequency_x_;
  }
  void set_grid_frequency(double frequency) {
    set_grid_frequency_x(frequency);
  }

 private:
  Color color_;
  GridType grid_type_{GridType::None};
  double grid_frequency_x_{
    5.0};  // Horizontal cells (rectangle) or radial lines (circle/ellipse)
  double grid_frequency_y_{
    5.0};  // Vertical cells (rectangle) or concentric circles (circle/ellipse)
};
