...
DocumentModel::DocumentModel()
    ...
    substrate_->on_changed().connect([this](const ModelChange &change){
        notify_all(change);
    });
}

std::shared_ptr<ShapeModel> DocumentModel::create_shape(ShapeModel::ShapeType type, const std::string &name) {
    auto shape = std::make_shared<ShapeModel>(type);
    if (!name.empty()) {
        shape->set_name(name);
    }
    shape->on_changed().connect([this](const ModelChange &change){
        notify_all(change);
    });
...
std::shared_ptr<MaterialModel> DocumentModel::create_material(const Color &color, const std::string &name) {
    auto material = std::make_shared<MaterialModel>(color);
    if (!name.empty()) {
        material->set_name(name);
    }
    material->on_changed().connect([this](const ModelChange &change){
        notify_all(change);
    });
...
void DocumentModel::set_substrate(const std::shared_ptr<SubstrateModel> &substrate) {
...
void DocumentModel::remove_material(const std::shared_ptr<MaterialModel> &material) {
